...
uses
  System.SysUtils, System.Generics.Collections, JOSE.Types.Bytes, JOSE.Core.Base, JOSE.Core.Parts, JOSE.Core.JWT, JOSE.Context;
...
begin
  if AllowedClockSkewSeconds > 0 then
    Result := Format('(even when providing [%d] seconds of leeway to account for clock skew)', [AllowedClockSkewSeconds])
  else
    Result := '.';
end;
...
begin
  Result := function (AJOSEContext: TJOSEContext): string
            var
              LClaims: TJWTClaims;
            ...
            begin
              ...
              begin
                if ADateParams.EvaluationTime.IsAfter(LExpiration, ADateParams.AllowedClockSkewSeconds) then
                  Exit(Format('The JWT is no longer valid - the evaluation time [%s] is on or after the Expiration Time [exp=%s] claim value %s', [ADateParams.EvaluationTime.AsISO8601, DateToISO8601(LClaims.Expiration, False), ADateParams.SkewMessage]));

                if LClaims.HasIssuedAt and LExpiration.IsBefore(LIssuedAt, ADateParams.AllowedClockSkewSeconds) then
                  Exit(Format('The Expiration Time (exp=%s) claim value cannot be before the IssuedAt (iat=%s) claim value', [LExpiration.AsISO8601, LIssuedAt.AsISO8601]));

                if LClaims.HasNotBefore and LExpiration.IsBefore(LNotBefore, ADateParams.AllowedClockSkewSeconds) then
                  Exit(Format('The Expiration Time (exp=%s) claim value cannot be before the NotBefore (nbf=%s) claim value', [LExpiration.AsISO8601, LNotBefore.AsISO8601]));

                if ADateParams.MaxFutureValidityInMinutes > 0 then
                begin
                  LDeltaInSeconds := LExpiration.AsSeconds - ADateParams.AllowedClockSkewSeconds - ADateParams.EvaluationTime.AsSeconds;

                  if LDeltaInSeconds > (ADateParams.MaxFutureValidityInMinutes * 60) then
                    Exit(Format('The Expiration Time [exp=%s] claim value cannot be more than [%d] minutes in the future relative to the evaluation time [%s] %s', [LExpiration.AsISO8601, ADateParams.MaxFutureValidityInMinutes, ADateParams.EvaluationTime.AsISO8601, ADateParams.SkewMessage]));
                end;
              end;
              ...
              if LClaims.HasNotBefore then
                if (ADateParams.EvaluationTime.AsSeconds + ADateParams.AllowedClockSkewSeconds) < LNotBefore.AsSeconds then
                  Exit(Format('The JWT is not yet valid as the evaluation time [%s] is before the NotBefore [nbf=%s] claim time %s', [ADateParams.EvaluationTime.AsISO8601, LNotBefore.AsISO8601, ADateParams.SkewMessage]));
            end;;
end;
  ...
  Result := function (AJOSEContext: TJOSEContext): string
            ...
            begin
              ...
              if not AJOSEContext.GetClaims.HasIssuer and ARequired then
                Exit(Format('No Issuer [iss] claim present but was expecting %s', [AIssuers.ToStringPluralForm('one of ')]));

              if (AIssuers.Size > 0) and not AIssuers.Contains(LIssuer) then
                  Exit(Format('Issuer [iss] claim value [%s] doesn''t match expected value of [%s]', [LIssuer, AIssuers.ToStringPluralForm('one of ')]));
            end;
end;
  ...
  Result := function (AJOSEContext: TJOSEContext): string
            ...
            begin
              ...
              if not AJwtId.IsEmpty and not AJwtId.Equals(LJwtId) then
                Exit(Format('JWT Id [jti] claim value [%s] doesn''t match expected value of [%s]', [LJwtId, AJwtId]));
            end;
end;
  ...
  Result := function (AJOSEContext: TJOSEContext): string
            ...
            begin
              ...
              if not ASubject.IsEmpty and not ASubject.Equals(LSubject) then
                Exit(Format('Subject [sub] claim value [%s] doesn''t match expected value of [%s]', [LSubject, ASubject]));
            end;
end;
