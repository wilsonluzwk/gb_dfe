...
uses
  System.SysUtils, JOSE.Types.Bytes, JOSE.Hashing.HMAC, JOSE.Signing.RSA, JOSE.Signing.ECDSA, JOSE.Core.Base, JOSE.Core.Parts,
  JOSE.Core.JWA, JOSE.Core.JWK;
...
uses
  System.Types, System.StrUtils, JOSE.Encoding.Base64;
...
begin
  ...
  if AKey.Size * 8 < FKeyMinLength then
    raise EJOSEException.CreateFmt('Key is too short (%dbit), expected (%dbit)', [AKey.Size * 8, FKeyMinLength]);
end;
...
begin
  if not AKey.IsEmpty then
    raise EJOSEException.Create(Format(CANNOT_HAVE_KEY, [THeaderNames.ALGORITHM, TJOSEAlgorithmId.None.AsString]));
end;
